...
alc %>% group_by(sex) %>% summarise(count = n()) # female = 195, male = 175

# calculate means of the grades of students
alc %>% summarise(mean_grade = mean(G3)) # mean_grade = 11.51622

# produce summary statistics by group again
alc %>% group_by(sex, high_use) %>% summarise(count = n()) # high_use female = 41, not = 154; high_use male = 70, not = 105
...
## Male Students with high alcohol use consistently show higher or lower absences compared to those with non-high alcohol use, it suggests a potential connection. Female students does not show similar pattern.
...
# add "sex" as explanatory variable to the model
...
cbind(OR, CI) ## sex has the widest interval, ranging from 1.68 to 4.44. No intervals contain 1. Confidence interval contains 1 indicates that odds ration is not significantly different from 1, which means that two groups events have the same probability and there is no difference between two groups.
...
- Execute the call to the loss function with `prob = 0`, meaning you define the probability of `high_use` as zero for each individual. What is the interpretation of the resulting proportion? (0.3)
When prob = 0, it means that the predicted probability of high_use is always 0. The mean prediction erro is 0.3, representing 30% of the predictions are incorrect when predicting all instances as not high_use.


- Adjust the code: change the `prob` argument in the loss function to `prob = 1`. What kind of a prediction does this equal to? What is the interpretation of the resulting proportion? (0.7)
When prob = 1, the resulting proportion is 0.7. It means that, on average, 70% of the predictions are incorrect when predicting all instances as high-use.

- Adjust the code again:  change the `prob` argument by giving it the prediction probabilities in `alc` (the column `probability`). What is the interpretation of the resulting proportion? (0.2297297)
When prob = alc$probability, the resulting proportion is 0.2297297. It means that, on average, about 22.97% of the predictions made by the model are incorrect. 
...
Mean error prediction for testing data is 0.2567568 (0.2324324). The prediction error is higher on the testing data compared to training data. The reason might be that the model learned some data that are not representative of the true underlying relationship, or the dataset is not big enough.
Hint:
...
# adjust the code to perfome 10-fold cross-validation
