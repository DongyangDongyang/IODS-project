...
# explore the data set # 506 observations and 14 variables. The data type is mostly numberic except chas and rad are integer.
str(Boston)
summary(Boston)
...
- Look at the table. Did the classifier predict the crime rates correctly? (73.53% of predictions are correct. )
...
### Instructions
- Load the MASS package and the `Boston` data set from it
- Create `dist_eu` by calling the `dist()` function on the Boston data set. Note that by default, the function uses Euclidean distance measure.
